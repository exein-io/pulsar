...
pub(crate) struct InterestMap(pub(crate) HashMap<MapRefMut, i32, u8>);

impl InterestMap {
    /// Try to load the map from eBPF
    pub(crate) fn load(bpf: &mut Bpf) -> Result<Self, MapError> {
...
pub(crate) struct RuleMap(HashMap<MapRefMut, Image, u8>);
...
impl RuleMap {
    ...
    pub(crate) fn whitelist(bpf: &mut Bpf) -> Result<Self, MapError> {
        let map = HashMap::try_from(bpf.map_mut("whitelist")?)?;
        Ok(Self(map))
    }

    /// Try to load the target map
    pub(crate) fn target(bpf: &mut Bpf) -> Result<Self, MapError> {
