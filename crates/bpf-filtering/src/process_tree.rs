...
pub(crate) enum Error {
    ...
    InvalidIteratorOutputFormat(usize),
    ...
    pub(crate) fn load_from_bpf_iterator(bpf: &mut Ebpf) -> Result<Self, Error> {
        ...
        for line in lines {
            ...
            if parts.len() != 12 && parts.len() != 14 {
                return Err(Error::InvalidIteratorOutputFormat(parts.len()));
            }
            ...
            let ppid: i32 = parts[3].parse()?;
            let parent = Pid::from_raw(ppid);

            let uts: u32 = parts[4].parse()?;
            let ipc: u32 = parts[5].parse()?;
            let mnt: u32 = parts[6].parse()?;
            let pid_ns: u32 = parts[7].parse()?;
            let net: u32 = parts[8].parse()?;
            let time: u32 = parts[9].parse()?;
            let cgroup: u32 = parts[10].parse()?;
            ...
            let image = parts[11].to_owned();

            let container_id = if parts.len() == 13 {
                let container_engine: u32 = parts[11].parse()?;
                let container_engine = match container_engine {
                    0 => ContainerEngineKind::Docker,
                    1 => ContainerEngineKind::Podman,
                    _ => return Err(Error::UnknownContainerEngine(container_engine)),
                };
                let container_id = parts[12].to_owned();
