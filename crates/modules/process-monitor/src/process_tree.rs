...
impl ProcessTree {
    pub(crate) fn load_from_bpf_iterator(bpf: &mut Ebpf) -> Result<Self, ProcessTreeError> {
    ...
    pub(crate) fn fork(
    ...
    ) -> Result<&ProcessData, ProcessTreeError> {
        ...
        match parent {
            ...
            None => Err(ProcessTreeError::ParentNotFound { pid, ppid }),
        }
    }

    pub(crate) fn exec(&mut self, pid: Pid, image: &str) -> Result<&ProcessData, ProcessTreeError> {
        match self.processes.iter().position(|p| p.pid == pid) {
            Some(i) => {
                self.processes[i].image = image.to_string();
                Ok(&self.processes[i])
            }
            None => Err(ProcessTreeError::ProcessNotFound { pid }),
        }
    }
}
